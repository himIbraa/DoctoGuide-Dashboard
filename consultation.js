...
const renderConsultationList = async () => {
    let consultation = document.getElementById("consultation");
    let dr = "";
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentPageData = consultationData.slice(startIndex, endIndex);

    for (const item of currentPageData) {
        const doctorRes = await database.from("doctor").select("name", "speciality", "picture").eq("id_doctor", item.id_doctor).single();
        const patientRes = await database.from("patient").select("name", "picture").eq("id_patient", item.patient_id).single();
...
const searchConsultation = () => {
    ...
    const rows = document.getElementById("consultation").getElementsByTagName("tr");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const searchFormConsultation = document.getElementById("searchFormConsultation");
    searchFormConsultation.addEventListener("submit", function (event) {
        event.preventDefault();
        searchConsultation();
    });
});
